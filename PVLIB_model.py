...
class Photovoltaic():
         def __init__(self, latitude, longitude, altitude, timezone, m_azimut, m_tilt, module_number, time, dni, ghi, dhi, albedo, a_r, irrad_model, module, temp_amb, wind_amb, pressure):
            ...
            self.IAM_mod_diff = pvlib.iam.martin_ruiz_diffuse(self.m_tilt, a_r=self.a_r, c1=0.4244, c2=None)        #Estimating Angle of incidence modifier(IAM) using the Martin and Ruiz for diffuse radiation
            
            self.F1 = pvlib.pvsystem.sapm_spectral_loss(self.am_abs, self.module)                                    #Calculating SAPM spectral loss coefficient, F1.
                       
            self.IAM_mod_dir = pvlib.iam.martin_ruiz(self.aoi, a_r=self.a_r)                                        #Estimating Angle of incidence modifier(IAM) using the Martin and Ruiz for direct radiation
            ...
            self.effective_irradiance  = self.F1 * (self.total_irrad['poa_direct'] * self.IAM_mod_dir 
                                                    ...
                                                    + self.IAM_mod_diff[1] * self.total_irrad['poa_ground_diffuse'])   #Effective Irradiance calculation --> Ground reflection considered
