...
class Photovoltaic():
         def __init__(self, latitude, longitude, altitude, timezone, m_azimut, m_tilt, module_number, time, dni, ghi, dhi, albedo, a_r, irrad_model, module, temp_amb, wind_amb, pressure):
            ...
            self.tcell = pvlib.temperature.faiman(
                self.total_irrad['poa_global'],                                 # Total incident irradiance [W/m^2]
                temp_amb,                                                       # Ambient dry bulb temperature [C]
                wind_amb,                                                       # Wind speed in m/s measured at the same height for which the wind loss factor was determined. 
                u0=25.0,                                                        # Combined heat loss factor coefficient [(W/m^2)/C]. The default value is one determined by Faiman for 7 silicon modules.
                u1=6.84)                                                        # Combined heat loss factor influenced by wind [(W/m^2)/(C)]
