...
dwd_data = pd.read_excel(os.path.join("Imports",
                                      r'Timeseries_JRC_PVGIS_TH_Koeln.xlsx'))  # Hourly Weather Data (DNI , GHI , DHI , temp_air , wind speed and pressure)
...
op_strategy = op_strategy.reindex(
    columns=['Operating_Strategy', 'Volume_Flow_[m3/h]', 'Mass_Flow_[kg/s]', 'Cumulative_Pressure_Drop_[Pa]',
             'Irradiance_[W/m2]', 'Ambient_temperature [°C]', 'wind_speed [m/s]'])
...
for i in range(len(op_strategy)):
    m_flow_loss_temp = pd.read_excel(os.path.join("Imports", r'rauhigkeit.xlsx'), sheet_name=os_name.format(str(i)),
                                     usecols="A,I:J, L, R")
...
mass_flow_loss = mass_flow_loss.select_dtypes(exclude='object').div(1.3).combine_first(
    mass_flow_loss)  # all values for the leakge mass flow in the valves has to be divided by 1.3
...
for i in tqdm(pv_data.index[0:8759]):  # One Year Sim.: [0:8759]
    ...
    if int(electrical_yield.effective_irradiance) == 0:
        ...
        C_k_annual = 1 + (module["gamma_pmp"] / 100) * (
                    electrical_yield.tcell.item() - 31.01)  # PR temperature corrected - 25.14 °C is the mean module temperature over one year for every hour of irradiance (where the moduel are in operation) - without cooling effect

        # Ideal energy yield under STC conditions and temperature corrected
        E_ideal_STC = round((((P_PV_STC * C_k_STC * (num_sdp_series * num_sdp_parallel)) * float(
            electrical_yield.effective_irradiance)) / 1000), 2)
        ...
        PR_annual_eq = round(electrical_yield.power_ac / (((P_PV_STC * C_k_annual * (
                    num_sdp_series * num_sdp_parallel)) * float(electrical_yield.effective_irradiance)) / 1000), 2)
    ...
    P_MP = round(electrical_yield.annual_energy, 2) / (
                num_sdp_series * num_sdp_parallel)  # Anual Energy / total modules
    ...
    while totalLoops < 1:
        ...
        if int(electrical_yield_new.effective_irradiance) == 0:
            ...
            efficency_New = round(
                (round(electrical_yield_new.annual_energy, 2) / (num_sdp_series * num_sdp_parallel * 0.1)) * 100 / int(
                    electrical_yield_new.effective_irradiance), 2)
        ...
        if int(electrical_yield_new.effective_irradiance) == 0:
            ...
            C_k_annual = 1 + (module["gamma_pmp"] / 100) * (
                        t_cooling - 28.03)  # PR temperature corrected - 25.14 °C is the mean module temperature over one year for every hour of irradiance (where the moduel are in operation) - taking into account the cooling effect

            # Ideal energy yield under STC conditions and temperature corrected
            E_ideal_STC = round((((P_PV_STC * C_k_STC * (num_sdp_series * num_sdp_parallel)) * float(
                electrical_yield_new.effective_irradiance)) / 1000), 2)
            ...
            PR_annual_eq = round(electrical_yield_new.power_ac / (((P_PV_STC * C_k_annual * (
                        num_sdp_series * num_sdp_parallel)) * float(electrical_yield_new.effective_irradiance)) / 1000),
                                 2)
    ...
    flux = round((m_out * 1.005 * (t_heatflux_out - temp_amb) / (num_sdp_series * num_sdp_parallel * 0.10)),
                 3)  # cp_air: 1.005 kJ/kg*K, Unit is kJ/s --> kW
...
column_values_elec = ["Index", "Time", "Tamb [°C]", "Tmcooling [°C]", "Tm [°C]", "T heatflux [°C]", "Power-DC [W]",
                      "Power-AC [W]", "Effective Irradiance [W/m^2]", "Elec. Efficency [%]",
                      "ideal elec. energy yield [Wh]", "Performance Ratio [-]", "Performance Ratio STC[-]",
                      "Performance Ratio eq [-]", "spez. elec. energy yield [kWh/kWp]", "Autarky rate [%]",
                      "HP_COP [-]", "HP_Thermal[Wh]"]
...
Jahresarbeitszahl_ref = round(
    electrical_data_New.select_dtypes(np.number).sum().loc['HP_Thermal[Wh]'] / (P_HP * len(electrical_data_New)), 2)
Jahresarbeitszahl = round(
    electrical_data_New.select_dtypes(np.number).sum().loc['HP_Thermal[Wh]'] / (P_HP * len(electrical_data_New)), 2)
